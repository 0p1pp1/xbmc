  ...
  else if(demuxer)
  {
    ...
    for (auto stream : demuxer->GetStreams())
    {
      ...
      if(stream->type == STREAM_AUDIO)
      {
        ...
        s.is_dmono = static_cast<CDemuxStreamAudio*>(stream)->bIsDmono;
        s.dmono_mode = static_cast<EDMONOMODE>(
                          static_cast<CDemuxStreamAudio*>(stream)->iDmonoMode);
        if (s.is_dmono)
        {
          s.channels = 2;
          s.language2 = g_LangCodeExpander.ConvertToISO6392T(
                          static_cast<CDemuxStreamAudio*>(stream)->sublang);
        }
      }
      Update(s);
    }
...
void CVideoPlayer::SetAudioDmonoMode(EDMONOMODE mode)
{
  ...
  m_CurrentAudio.hint.dmono_mode = static_cast<int>(mode);

  CDemuxStreamAudio* stream;
  stream = static_cast<CDemuxStreamAudio*>(
            m_pDemuxer->GetStream(m_CurrentAudio.demuxerId, m_CurrentAudio.id));
  if (stream)
    stream->iDmonoMode = static_cast<int>(mode);

  m_VideoPlayerAudio->SetDmonoMode(static_cast<int>(mode));
}
...
void CVideoPlayer::GetAudioStreamInfo(int index, AudioStreamInfo &info)
{
  ...
  info.dmono_mode = static_cast<int>(s.dmono_mode);
