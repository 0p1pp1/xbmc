  ...
  if (g_application.GetAppPlayer().IsPlayingVideo())
  {
    switch (action.GetID())
    {
      ...
      case ACTION_AUDIO_NEXT_LANGUAGE:
      {
        ...
        if (info.is_dmono && info.dmono_mode == static_cast<int>(DMONO_LEFT))
        {
          g_application.GetAppPlayer().SetAudioDmonoMode(DMONO_RIGHT);
          language = info.language2;
        }
        else 
        if (!info.is_dmono && g_application.GetAppPlayer().GetAudioStreamCount() == 1)
          return true;
        else
        {
          ...
          g_application.GetAppPlayer().GetAudioStreamInfo(currentAudio, info);
          g_application.GetAppPlayer().SetAudioDmonoMode(DMONO_LEFT);
